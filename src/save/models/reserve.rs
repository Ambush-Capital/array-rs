...
impl Reserve {
    ...
    pub fn borrow_weight(&self) -> Decimal {
        Decimal::one().try_add(Decimal::from_bps(self.config.added_borrow_weight_bps)).unwrap()
    }

    pub fn slot_adjustment_factor(&self) -> Rate {
    ...
    //     let compounds_per_year = 365;
    //     let rate_per_period = apr.to_scaled_val::<f64>() / compounds_per_year as f64;
    ...
    //     let protocol_take_rate = Fraction::from_num(1.0 - self.get_protocol_take_rate().to_num::<f64>());
    ...
    pub fn price_scale(&self) -> Decimal {
        let scaled_price_offset_bps = self
            .config
            .scaled_price_offset_bps
            .clamp(MIN_SCALED_PRICE_OFFSET_BPS, MAX_SCALED_PRICE_OFFSET_BPS);
    ...
    pub fn price_lower_bound(&self) -> Decimal {
        let price =
            std::cmp::min(self.liquidity.market_price, self.liquidity.smoothed_market_price);
    ...
    pub fn market_value(&self, liquidity_amount: Decimal) -> Result<Decimal, ProgramError> {
        self.liquidity.market_price.try_mul(liquidity_amount)?.try_div(Decimal::from(
            (10u128)
                .checked_pow(self.liquidity.mint_decimals as u32)
                .ok_or(LendingError::MathOverflow)?,
        ))
    ...
    pub fn market_value_upper_bound(
        ...
        self.price_upper_bound().try_mul(liquidity_amount)?.try_div(Decimal::from(
            (10u128)
                .checked_pow(self.liquidity.mint_decimals as u32)
                .ok_or(LendingError::MathOverflow)?,
        ))
    ...
    pub fn market_value_lower_bound(
        ...
        self.price_lower_bound().try_mul(liquidity_amount)?.try_div(Decimal::from(
            (10u128)
                .checked_pow(self.liquidity.mint_decimals as u32)
                .ok_or(LendingError::MathOverflow)?,
        ))
    ...
    pub fn current_borrow_rate(&self) -> Result<Rate, ProgramError> {
        ...
        if utilization_rate <= optimal_utilization_rate {
            ...
            //borrow = ((0.8093-0.80)/(0.90-0.80))*(20-7)+7
    ...
    pub fn calculate_borrow(
        ...
        if amount_to_borrow == u64::MAX {
            ...
            let (borrow_fee, host_fee) =
                self.config.fees.calculate_borrow_fees(borrow_amount, FeeCalculation::Inclusive)?;
            let receive_amount = borrow_amount
                .try_floor_u64()?
                .checked_sub(borrow_fee)
                .ok_or(LendingError::MathOverflow)?;

            Ok(CalculateBorrowResult { borrow_amount, receive_amount, borrow_fee, host_fee })
        } else {
            let receive_amount = amount_to_borrow;
            let borrow_amount = Decimal::from(receive_amount);
            let (borrow_fee, host_fee) =
                self.config.fees.calculate_borrow_fees(borrow_amount, FeeCalculation::Exclusive)?;

            let borrow_amount = borrow_amount.try_add(borrow_fee.into())?;
            let borrow_value =
                self.market_value_upper_bound(borrow_amount)?.try_mul(self.borrow_weight())?;
    ...
    pub fn calculate_repay(
        ...
        Ok(CalculateRepayResult { settle_amount, repay_amount })
    }
    ...
    pub fn calculate_liquidation(
        ...
        if liquidity.market_value <= Decimal::one() {
            ...
            let liquidation_amount = obligation.max_liquidation_amount(liquidity)?.min(max_amount);
            let liquidation_pct = liquidation_amount.try_div(liquidity.borrowed_amount_wads)?;
            let liquidation_value =
                liquidity.market_value.try_mul(liquidation_pct)?.try_mul(bonus_rate)?;
        ...
        Ok(CalculateLiquidationResult { settle_amount, repay_amount, withdraw_amount })
    }
    ...
    pub fn calculate_protocol_liquidation_fee(
        ...
        Ok(std::cmp::max(
            nonbonus_amount.try_mul(bonus.protocol_liquidation_fee)?.try_ceil_u64()?,
            1,
        ))
    }
    ...
    pub fn calculate_redeem_fees(&self) -> Result<u64, ProgramError> {
        Ok(min(
            ...
            self.liquidity.accumulated_protocol_fees_wads.try_floor_u64()?,
        ))
    }
}
...
impl CollateralExchangeRate {
    ...
    pub fn collateral_to_liquidity(&self, collateral_amount: u64) -> Result<u64, ProgramError> {
        self.decimal_collateral_to_liquidity(collateral_amount.into())?.try_floor_u64()
    }
    ...
    pub fn liquidity_to_collateral(&self, liquidity_amount: u64) -> Result<u64, ProgramError> {
        self.decimal_liquidity_to_collateral(liquidity_amount.into())?.try_floor_u64()
    }
...
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, FromPrimitive, Hash)]
    ...
    fn calculate_fees(
        ...
        if borrow_fee_rate > Rate::zero() && amount > Decimal::zero() {
            ...
            let host_fee = if need_to_assess_host_fee {
                borrow_fee_decimal.try_mul(host_fee_rate)?.try_round_u64()?.max(1u64)
            } else {
                0
            };
...
impl Pack for Reserve {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        ...
        *liquidity_available_amount = self.liquidity.available_amount.to_le_bytes();
        pack_decimal(self.liquidity.borrowed_amount_wads, liquidity_borrowed_amount_wads);
        pack_decimal(
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        let max_liquidation_bonus =
            max(liquidation_bonus, u8::from_le_bytes(*config_max_liquidation_bonus));
        let liquidation_threshold = u8::from_le_bytes(*config_liquidation_threshold);
        let max_liquidation_threshold =
            max(liquidation_threshold, u8::from_le_bytes(*config_max_liquidation_threshold));
...
mod test {
    use super::*;
    use crate::save::math::{PERCENT_SCALER, WAD};
    ...
    fn pack_and_unpack_reserve() {
        ...
        for _ in 0..100 {
            ...
            let extra_oracle_pubkey =
                if rng.gen_bool(0.5) { Some(Pubkey::new_unique()) } else { None };
            let extra_market_price =
                if extra_oracle_pubkey.is_some() { Some(rand_decimal()) } else { None };
    ...
    fn borrow_fee_calculation_min_host() {
        ...
        let err =
            fees.calculate_borrow_fees(Decimal::from(2u64), FeeCalculation::Exclusive).unwrap_err();
        assert_eq!(err, LendingError::BorrowTooSmall.into()); // minimum of 3 tokens

        // only 1 token borrowed, get error
        let err =
            fees.calculate_borrow_fees(Decimal::one(), FeeCalculation::Exclusive).unwrap_err();
        assert_eq!(err, LendingError::BorrowTooSmall.into());

        // 0 amount borrowed, 0 fee
        let (total_fee, host_fee) =
            fees.calculate_borrow_fees(Decimal::zero(), FeeCalculation::Exclusive).unwrap();
    ...
    fn borrow_fee_calculation_min_no_host() {
        ...
        let (total_fee, host_fee) =
            fees.calculate_borrow_fees(Decimal::from(2u64), FeeCalculation::Exclusive).unwrap();
    ...
    fn borrow_fee_calculation_host() {
        ...
        let (total_fee, host_fee) =
            fees.calculate_borrow_fees(Decimal::from(1000u64), FeeCalculation::Exclusive).unwrap();
    ...
    fn calculate_protocol_liquidation_fee() {
        let reserve = Reserve {
            config: ReserveConfig { protocol_liquidation_fee: 10, ..Default::default() },
            ..Reserve::default()
        };
    ...
    fn market_value() {
        ...
        assert_eq!(
            reserve.market_value(Decimal::from(10 * LAMPORTS_PER_SOL)).unwrap(),
            Decimal::from(250u64)
        );

        assert_eq!(
            reserve.market_value_lower_bound(Decimal::from(10 * LAMPORTS_PER_SOL)).unwrap(),
            Decimal::from(250u64)
        );

        assert_eq!(
            reserve.market_value_upper_bound(Decimal::from(10 * LAMPORTS_PER_SOL)).unwrap(),
            Decimal::from(500u64)
        );
    ...
    fn usd_to_liquidity_amount_lower_bound() {
        ...
        assert_eq!(
            reserve.usd_to_liquidity_amount_lower_bound(Decimal::from(100u64)).unwrap(),
    ...
    fn reserve_config_test_cases() -> impl Strategy<Value = ReserveConfigTestCase> {
        prop_oneof![
            ...
            Just(ReserveConfigTestCase {
                config: ReserveConfig { protocol_liquidation_fee: 51, ..ReserveConfig::default() },
                result: Err(LendingError::InvalidConfig.into()),
            }),
            Just(ReserveConfigTestCase {
                config: ReserveConfig { scaled_price_offset_bps: 2001, ..ReserveConfig::default() },
                result: Err(LendingError::InvalidConfig.into()),
            }),
            Just(ReserveConfigTestCase {
                config: ReserveConfig {
