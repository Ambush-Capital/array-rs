...
enum EnumVariant {
    ...
    Complex { name: String, fields: Vec<StructField> },
    Simple { name: String },
}
...
fn generate_idl_types(idl: &Idl) -> String {
    ...
    for type_def in &idl.types {
        let type_name =
            Ident::new(&capitalize_first_letter(&type_def.name), proc_macro2::Span::call_site());
        let type_tokens = match &type_def.type_def {
            TypeData::Enum { variants } => {
                ...
                let variant_tokens =
                    variants.iter().enumerate().map(|(i, variant)| match variant {
    ...
    for account in &idl.accounts {
        ...
        let discriminator: TokenStream =
            format!("{:?}", sighash("account", &account.name)).parse().unwrap();
    ...
    let custom_types: TokenStream =
        include_str!("custom_types.rs").parse().expect("custom_types valid rust");
...
fn format_rust_code(code: &str) -> String {
    ...
    let output = rustfmt.wait_with_output().expect("Failed to read rustfmt output");
