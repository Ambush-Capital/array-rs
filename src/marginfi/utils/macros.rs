...
macro_rules! check {
    ($cond:expr, $err:expr) => {
        if !($cond) {
            let error_code: $crate::marginfi::utils::errors::MarginfiError = $err;
...
macro_rules! math_error {
    () => {{
        || {
            let error_code = $crate::marginfi::utils::errors::MarginfiError::MathError;
            anchor_lang::prelude::msg!(
                "Error \"{}\" thrown at {}:{}",
                error_code,
                file!(),
...
macro_rules! bank_signer {
    ($vault_type: expr, $bank_pk: expr, $authority_bump: expr) => {
        &[&[$vault_type.get_authority_seed().as_ref(), &$bank_pk.to_bytes(), &[$authority_bump]]]
    };
}
...
macro_rules! live {
    () => {
        cfg!(any(feature = "mainnet-beta", feature = "staging", feature = "devnet"))
    };
}
