use super::{
    account::{BalanceSide, RequirementType},
    price::OracleSetup,
};
...
use crate::{
    ...
    marginfi::utils::{
        constants::{
            ...
            LIQUIDITY_VAULT_SEED, MAX_ORACLE_KEYS, TOTAL_ASSET_VALUE_INIT_LIMIT_INACTIVE,
        },
...
impl InterestRateConfig {
    ...
    pub fn validate(&self) -> MarginfiResult {
        ...
        check!(optimal_ur > I80F48::ZERO && optimal_ur < I80F48::ONE, MarginfiError::InvalidConfig);
...
impl InterestRateCalc {
    ...
    pub fn calc_interest_rate(&self, utilization_ratio: I80F48) -> Option<ComputedInterestRates> {
        ...
        let borrowing_rate_apr =
            base_rate.checked_mul(I80F48::ONE.checked_add(fee_ir)?)?.checked_add(fee_fixed)?;
...
impl ComputedInterestRates {
    pub fn borrowing_rate_apy(&self) -> I80F48 {
        // Convert APR to APY using the formula: APY = (1 + APR/n)^n - 1
        // Where n is number of compounding periods per year (using 31536000 seconds)
        let one = I80F48::ONE;
        let n = I80F48::from_num(31536000i64);
        let apr_per_period = self.borrowing_rate_apr.checked_div(n).unwrap_or(I80F48::ZERO);

        self.pow(one.checked_add(apr_per_period).unwrap_or(one), 31536000)
            .checked_sub(one)
            .unwrap_or(I80F48::ZERO)
    }

    pub fn lending_rate_apy(&self) -> I80F48 {
        let one = I80F48::ONE;
        let n = I80F48::from_num(31536000i64);
        let apr_per_period = self.lending_rate_apr.checked_div(n).unwrap_or(I80F48::ZERO);
    ...
    fn pow(&self, base: I80F48, exp: u32) -> I80F48 {
        ...
        while exp > 0 {
            if exp & 1 == 1 {
                result = result.checked_mul(base).unwrap_or(I80F48::ZERO);
            }
            base = base.checked_mul(base).unwrap_or(I80F48::ZERO);
            exp >>= 1;
        }
        result
    }
...
#[derive(Debug, PartialEq, Eq, TypeLayout, AnchorDeserialize, AnchorSerialize, Default, Clone)]
...
impl Bank {
    ...
    pub fn get_liability_amount(&self, shares: I80F48) -> MarginfiResult<I80F48> {
        Ok(shares.checked_mul(self.liability_share_value.into()).ok_or_else(math_error!())?)
    }

    pub fn get_asset_amount(&self, shares: I80F48) -> MarginfiResult<I80F48> {
        Ok(shares.checked_mul(self.asset_share_value.into()).ok_or_else(math_error!())?)
    }

    pub fn get_liability_shares(&self, value: I80F48) -> MarginfiResult<I80F48> {
        Ok(value.checked_div(self.liability_share_value.into()).ok_or_else(math_error!())?)
    }

    pub fn get_asset_shares(&self, value: I80F48) -> MarginfiResult<I80F48> {
        Ok(value.checked_div(self.asset_share_value.into()).ok_or_else(math_error!())?)
    }

    pub fn get_interest_rate(&self, group: &MarginfiGroup) -> Option<ComputedInterestRates> {
        let calc = self.config.interest_rate_config.create_interest_rate_calculator(group);
        calc.calc_interest_rate(self.get_utilization_ratio().unwrap())
    }

    pub fn get_utilization_ratio(&self) -> Result<I80F48> {
    ...
    pub fn get_total_borrowed(&self) -> Result<I80F48> {
        let total_liabilities = self.get_liability_amount(self.total_liability_shares.into())?;

        Ok(total_liabilities)
    }

    pub fn get_total_supply(&self) -> Result<I80F48> {
        let total_assets = self.get_asset_amount(self.total_asset_shares.into())?;
...
//     debug!("Utilization rate: {}, time delta {}s", utilization_rate, time_delta);
...
//         value.checked_mul(apr)?.checked_mul(time_delta.into())?.checked_div(SECONDS_PER_YEAR)?;
    ...
    pub fn get_weights(&self, req_type: RequirementType) -> (I80F48, I80F48) {
        match req_type {
            ...
            RequirementType::Maintenance => {
                (self.asset_weight_maint.into(), self.liability_weight_maint.into())
            }
...
#[repr(C, align(8))]
#[derive(Default, AnchorSerialize, AnchorDeserialize, Zeroable, Copy, Clone, Pod, TypeLayout)]
...
#[derive(Clone, Copy, AnchorDeserialize, AnchorSerialize, Debug, PartialEq, Eq, TypeLayout)]
...
mod tests {
    ...
    fn calc_fee_rate_1() {
        ...
        assert_eq!(calc_fee_rate(rate, fee_ir, fee_fixed).unwrap(), I80F48!(0.03));
    }
