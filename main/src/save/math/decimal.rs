...
impl Decimal {
    ...
    pub fn try_floor_u64(&self) -> Result<u64, ProgramError> {
        let ceil_val = self.0.checked_div(Self::wad()).ok_or(LendingError::MathOverflow)?;
        Ok(u64::try_from(ceil_val).map_err(|_| LendingError::MathOverflow)?)
    }
}
...
impl TryDiv<u64> for Decimal {
    fn try_div(self, rhs: u64) -> Result<Self, ProgramError> {
        Ok(Self(self.0.checked_div(U192::from(rhs)).ok_or(LendingError::MathOverflow)?))
    }
}
...
impl TryMul<u64> for Decimal {
    fn try_mul(self, rhs: u64) -> Result<Self, ProgramError> {
        Ok(Self(self.0.checked_mul(U192::from(rhs)).ok_or(LendingError::MathOverflow)?))
    }
}
...
mod test {
    ...
    fn test_to_scaled_val() {
        assert_eq!(Decimal(U192::from(u128::MAX)).to_scaled_val().unwrap(), u128::MAX);

        assert_eq!(
            Decimal(U192::from(u128::MAX)).try_add(Decimal(U192::from(1))).unwrap().to_scaled_val(),
    ...
    fn test_round_floor_ceil_u64() {
        ...
        val = val.try_add(Decimal::from_scaled_val(HALF_WAD as u128 - 1)).unwrap();
        assert_eq!(val.try_round_u64().unwrap(), 1);
        assert_eq!(val.try_floor_u64().unwrap(), 1);
    ...
    fn test_display() {
        ...
        assert_eq!(Decimal::from_scaled_val(1u128).to_string(), "0.000000000000000001");
    }

    #[test]
    fn test_saturating_sub() {
        assert_eq!(Decimal::from(1u64).saturating_sub(Decimal::from(2u64)), Decimal::zero());
        assert_eq!(Decimal::from(2u64).saturating_sub(Decimal::from(1u64)), Decimal::one());
    }
}
