...
use crate::kamino::{
    models::lending_result::LendingResult,
    models::reserve::{AssetTier, BigFractionBytes},
    utils::consts::ELEVATION_GROUP_NONE,
    utils::errors::LendingError,
    utils::fraction::{BigFraction, Fraction, FractionExtra, U256},
};

static_assertions::const_assert_eq!(0, std::mem::size_of::<Obligation>() % 8);
#[derive(PartialEq, Derivative, Zeroable, BorshSerialize, BorshDeserialize)]
#[derivative(Debug)]
#[repr(C)]
...
impl Display for Obligation {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        for collateral in self.deposits.iter().filter(|c| c.deposit_reserve != Pubkey::default()) {
            write!(
                f,
        ...
        for liquidity in self.borrows.iter().filter(|l| l.borrow_reserve != Pubkey::default()) {
            write!(
                f,
...
impl Obligation {
    ...
    ) -> Result<WithdrawResult, LendingError> {
    ...
    pub fn max_withdraw_value(&self, withdraw_collateral_ltv_pct: u8) -> LendingResult<Fraction> {
        ...
        Ok(allowed_borrow_value.saturating_sub(borrow_factor_adjusted_debt_value) * 100_u128
            / u128::from(withdraw_collateral_ltv_pct))
    }

    pub fn remaining_borrow_value(&self) -> Fraction {
        Fraction::from_bits(
            self.allowed_borrow_value_sf.saturating_sub(self.borrow_factor_adjusted_debt_value_sf),
        )
    }
    ...
    ) -> Result<&ObligationCollateral, LendingError> {
    ...
    pub fn position_of_collateral_in_deposits(
        &self,
        deposit_reserve: Pubkey,
    ) -> Result<usize, LendingError> {
        if self.deposits_empty() {
            return Err(LendingError::ObligationDepositsEmpty);
        }
        self.deposits
    ...
    pub fn deposits_empty(&self) -> bool {
        self.deposits.iter().all(|c| c.deposit_reserve == Pubkey::default())
    }

    pub fn borrows_empty(&self) -> bool {
        self.borrows.iter().all(|l| l.borrow_reserve == Pubkey::default())
    }

    pub fn deposits_count(&self) -> usize {
        self.deposits.iter().filter(|c| c.deposit_reserve != Pubkey::default()).count()
    }

    pub fn borrows_count(&self) -> usize {
        self.borrows.iter().filter(|l| l.borrow_reserve != Pubkey::default()).count()
    }
...
#[derive(
    Debug, Default, PartialEq, Eq, Zeroable, Copy, Clone, BorshSerialize, BorshDeserialize,
)]
#[repr(C)]
...
impl ObligationCollateral {
    ...
    pub fn deposit(&mut self, collateral_amount: u64) -> Result<(), LendingError> {
    ...
    pub fn withdraw(&mut self, collateral_amount: u64) -> Result<(), LendingError> {
        self.deposited_amount = self
            .deposited_amount
    ...
    pub fn accrue_interest(
        ...
        match new_cumulative_borrow_rate_bsf.cmp(&former_cumulative_borrow_rate_bsf) {
            ...
            Ordering::Greater => {
                ...
                self.borrowed_amount_sf =
                    borrowed_amount_sf_u256.try_into().map_err(|_| LendingError::MathOverflow)?;
